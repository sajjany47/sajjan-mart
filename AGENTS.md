...
- **Schema**: `Order` gained `refundStatus String?`, `razorpayOrderId String?`, `razorpayPaymentId String?`, `razorpaySignature String?`, `paidAt DateTime?`, `paymentFailureReason String?` (pushed via `prisma db push --accept-data-loss` + regenerated client — dev server restart needed to unlock the query-engine DLL). New **`PaymentSession`** table (`payment_sessions`): unique `razorpay_order_id`, optional `user_id`, `payload Json` (full checkout snapshot), `status` (`pending|processing|paid|failed|expired`), `order_id` link.
- **Orders are placed only after the payment succeeds.** Real Razorpay checkouts NEVER write to `orders` upfront — `POST /api/orders` creates the Razorpay order + a `payment_sessions` snapshot (idempotent upsert keyed on the razorpay order id) and returns only `{ key_id, demo_mode, razorpay_order_id }`. The DB order is created as `paid` (status `pending`) by `lib/razorpay-settle.ts` (`createOrderFromPaymentSession(session, {paymentId, signature})`): claims the session via `updateMany(orderId=null → processing)` so a client-verify vs webhook race settles exactly once, creates order + items from the payload, links `order_id`, sends the placed mails. A failed/dismissed checkout leaves NO order — the session just goes `failed`.
...
- **`POST /api/payments/verify`**: HMAC signature gate first, then (only when keys configured) a `payments.fetch` capture double-check. On success it settles the session → creates the paid order via `createOrderFromPaymentSession` and returns the order (with its `id`); replays that already settled are idempotent. Signature mismatch marks the session `failed`; no order is ever created for an unverified payment.
...
- **`POST /api/webhooks/razorpay`** (raw body + `x-razorpay-signature`): `payment.captured` settles the session into a paid order via the same helper (idempotent against a concurrent verify; falls back to marking an already-existing pending order paid + success mail), `payment.failed` marks the session `failed`, `refund.processed` sets `refundStatus='processed'`, bumps `refundedAmount` and flips `paymentStatus='refunded'` on full repayment. Orders are matched by the razorpay ids (non-unique columns). Unhandled events ack cleanly.
...
- **Checkout UI** (`components/store/checkout-client.tsx`): Razorpay Checkout script lazy-loaded via idempotent `loadRazorpayScript()`; on razorpay order created, `new Razorpay({key, amount, currency:'INR', order_id, prefill, theme, handler, modal:{ondismiss}})` opens; `handler` posts to `/api/payments/verify` (which returns the created order id); `payment.failed` posts to `/api/payments/failed`; `ondismiss` treats it as a cancelled payment. Cart is cleared and the customer is redirected to `/account/orders/<id>` ONLY when the payment verifies — a failed/dismissed checkout keeps the cart and no order exists. `demo_mode` skips the modal (instant paid; order returned at once). Payment note copy is method-aware (COD = pay on arrival; Razorpay = secure gateway line).
...
- **Verified**: `npx tsc --noEmit` clean, `npm run lint` clean (only pre-existing exhaustive-deps warnings), dev server restarted on :3000, HTTP smoke tests — razorpay demo order → `payment_status=paid`, `demo_mode=true`, no keys; COD → `payment_status=pending`; `/api/payments/verify` rejects a bogus signature (400). With real (test) keys in place: real razorpay checkout returns NO order id (session-only), orphaned checkout leaves 0 orders after failed verification, and the settle helper creates the paid order + items and is idempotent on repeat. Test data cleaned up.
