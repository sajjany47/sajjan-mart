...
export default function AdminPujasPage() {
  ...
  async function load() {
    ...
    const [pujasRes, productsRes] = await Promise.all([
      ...
      supabase
        .from("products")
        .select("id,name,sales_price,quantity_type")
        .eq("product_type", "puja_samagri")
        .eq("active", "true")
        .order("name"),
    ]);
  ...
  async function openEdit(p: Puja) {
    ...
    const { data } = await supabase
      ...
      .eq("puja_id", p.id);
    setSelectedItems(
      (data ?? []).map((i: any) => i.product_id).filter(Boolean),
    );
    setOpen(true);
  }
    ...
    <div>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            disabled={exporting}
            title="Download pujas.xlsx (Pujas + Puja Items sheets) — edit it in Excel and re-upload it here"
          >
            ...
            <div className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <Badge
                    className={`mt-2 ${p.is_active ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
                  >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Puja" : "New Puja"}</DialogTitle>
          </DialogHeader>
