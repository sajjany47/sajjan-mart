...
import {
  ...
  FormikCheckBox,
  FormikSelect,
} from "@/components/FormikTextInput";
...
function getValidationSchema(productType: string) {
  return Yup.object().shape({
    ...
    stock_type:
      productType !== "food"
        ? Yup.string().required("Stock type is required")
        : Yup.string(),
    stock:
      productType !== "food"
        ? Yup.number()
    ...
    food_type:
      productType === "food"
        ? Yup.string().required("Food type is required")
        : Yup.string(),
...
export default function AdminProductsPage() {
  ...
  const [sortBy, setSortBy] = useState("image-missing");
  ...
  const load = useCallback(async () => {
    ...
    const [productsRes, pujaItemsRes] = await Promise.all([
      supabase
        ...
        .eq("sort", sortBy),
      supabase.from("puja_items").select("*, puja(*)"),
    ]);
  ...
  useEffect(() => {
    supabase
      ...
      .select("*")
      .order("sort_order")
      .then(({ data }: any) => setCategories((data ?? []) as Category[]));
    supabase
      .from("brands")
      .select("*")
      .order("name")
  ...
  async function onSubmit(values: FormValues, { setSubmitting }: any) {
    if (values.product_type === "puja_samagri") {
      ...
      if (assigned.length === 0) {
        toast.error(
          "At least one puja assignment is required for puja samagri items.",
        );
        setSubmitting(false);
        return;
      }
    ...
    if (editing) {
      ...
      if (values.product_type === "puja_samagri") {
        await supabase.from("puja_items").delete().eq("product_id", editing.id);
  ...
  async function handleExport() {
    ...
    try {
      const res = await fetch(
        `/api/admin/export/puja-samagri?type=${activeTab}`,
      );
      if (!res.ok) {
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((p) => (
                      <tr key={p.id} className="border-t border-border">
                        ...
                        {t.value === "food" && (
                          <td className="px-4 py-3">
                            {FOOD_TYPES.find((f) => f.value === p.food_type)
                              ?.label ??
                              p.food_type ??
                              "-"}
                          </td>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              ...
              {
                PRODUCT_TYPES.find(
                  (t) => t.value === (editing?.product_type ?? activeTab),
                )?.label
              }
            </DialogTitle>
          </DialogHeader>
          <Formik
            initialValues={getInitialValues(
              editing?.product_type ?? activeTab,
              editing,
              categories,
            )}
...
function ProductFormContent({
  ...
  setPujaAssignments: (v: { puja_id: string; category: string }[]) => void;
}) {
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {pt === "natural" && (
              <Field
                ...
                placeholder="Select category"
                options={NATURAL_PRODUCT_CATEGORIES}
                component={FormikSelect}
              />
            )}
            {pt === "puja_samagri" && (
              <Field
                name="product_category"
                label="Puja Samagri Item"
                placeholder="Select item"
                options={PUJA_SAMAGRI_CATEGORIES}
                component={FormikSelect}
              />
            )}
          </div>
        ...
        <div className="space-y-3">
          <div className="flex gap-1 rounded-md border border-input p-1 w-fit">
            <button
              ...
              className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs font-medium transition ${imageInputMode === "url" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-xs font-medium transition ${imageInputMode === "upload" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}
            >
              ...
              {images.map((url, idx) => (
                <div key={idx} className="relative group">
                  ...
                  <button
                    ...
                    className="absolute -top-1.5 -right-1.5 bg-destructive text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition shadow-sm"
                  >
                    <X className="h-3 w-3" />
                  </button>
                  {idx === 0 && (
      ...
      {pt === "food" && (
        <div className="rounded-lg border border-border bg-muted/30 p-4">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Customers will be able to pick from these when adding this food to
            cart.
          </p>
        </div>
      )}
